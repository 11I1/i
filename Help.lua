...
Players.LocalPlayer:GetMouse().KeyDown:Connect(function(Key)
    if Key == 'r' then
        ...
        chr:SetPrimaryPartCFrame(CFrame.new(0, 5, football.Value.Position + football.Value.AssemblyLinearVelocity * 1.5) * CFrame.lookAt(chr.PrimaryPart.Position, football.Value.Position))
        Module:Slide()
        chr = nil
    end
end)
