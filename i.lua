...
insertCommand("svoid", function(getPlayer)
    ...
    plr.Character:PivotTo(CFrame.new(0, workspace.FallenPartsDestroyHeight, 0)); wait(1/3)
    clock = os.time()
...
insertCommand("lsvoid", function(getPlayer)
    ...
    api.cmds[api.prefix.new.."stop"]()
    loops.lsvoid = true
    while loops.lsvoid do api.cmds[api.prefix.new.."svoid"](getPlayer); plr.CharacterAdded:Wait(); task.wait(3/4) end
end)
