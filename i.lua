...
insertCommand("svoid", function(getPlayer)
    ...
    plr.Character:PivotTo(plr.Character:GetModelCFrame() * CFrame.new(0, -800, 0)); wait()
    ...
    plr.Character:PivotTo(plr.Character:GetModelCFrame() * CFrame.new(0, workspace.FallenPartsDestroyHeight, 0)); wait(1/4)
    clock = os.time()
    repeat task.wait()
        if not getPlayer.Character or not getPlayer.Character:FindFirstChildOfClass("Humanoid") or getPlayer.Character.Humanoid.Health <= 0 or (os.time() - clock) >= 5 then break end
