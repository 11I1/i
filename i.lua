...
task.defer(function()
    for i, v in next, api.cmds do Commands[#Commands + 1] = i:sub(2, #i) end
...
insertCommand("kill", function(p)
    if not findID or not status(plr) then return end
    ...
    for i = 1, #g do v = g[i] if v:IsA("BasePart") and v:FindFirstChildOfClass("TouchTransmitter") then a = v; break end end
    for i = 1, #j do v = j[i] if v:IsA("BasePart") and v:FindFirstChildOfClass("TouchTransmitter") then k = v; break end end
